...
def chamar_modelo(api_key: str, mensagens: list) -> str:
    """Chama o Groq (llama-3.3-70b) e retorna a resposta."""
...
MAX_CHARS_POR_DOC = 4000  # Limite por documento para não estourar o Groq

def ler_arquivo(uploaded_file) -> dict:
    """Lê arquivo e retorna tipo + conteúdo (texto truncado se necessário)."""
    ...
    if tipo == "application/pdf":
        ...
        # Imagens não são enviadas ao modelo — apenas registradas
        return {"tipo": "texto", "conteudo": f"[IMAGEM ENVIADA: {nome} — descreva o conteúdo relevante na conversa se necessário]"}

    elif tipo == "text/plain":
        texto = dados.decode("utf-8", errors="ignore")[:MAX_CHARS_POR_DOC]
...
def buscar_reputacao(nome_empresa: str) -> str:
    """Busca reputação no DuckDuckGo."""
    if not nome_empresa:
        return "Nome da empresa não informado — pesquisa não realizada."
    ...
    if not resultados:
        return f"Nenhuma ocorrência negativa encontrada para '{nome_empresa}'."
    return "\n".join(resultados[:9])
    ...
    with st.sidebar:
        ...
        if not api_key:
            api_key = st.text_input("Chave Groq API", type="password",
                                    help="Obtenha em console.groq.com — é gratuita")
    ...
    if not st.session_state.mensagens:
        ...
        with col2:
            if st.button("▶ Iniciar Análise", use_container_width=True, type="primary"):
                if not api_key:
                    st.error("Insira a chave da API Groq na barra lateral.")
                    return
                ...
                # Limita total de texto enviado ao modelo
                texto_total = "\n\n".join(textos)[:12000]
                contexto_docs = ""
                if textos:
                ...
                with st.spinner("Processando documentos..."):
                    try:
                        reply = chamar_modelo(api_key, msgs_api)
                    except Exception as e:
...
a estrutura definida, do cabeçalho até a CONCLUSÃO FINAL."""

                # Reconstrói histórico completo
                msgs_api = [{"role": "system", "content": SYSTEM_PROMPT}]
                for m in st.session_state.mensagens[:-1]:
                    role_api = "user" if m["role"] == "user" else "assistant"
                    msgs_api.append({"role": role_api, "content": m["texto"]})
                msgs_api.append({"role": "user", "content": prompt_final})

                with st.chat_message("assistant"):
                    with st.spinner("Gerando parecer completo..."):
                        try:
                            relatorio = chamar_modelo(api_key, msgs_api)
                        except Exception as e:
